...
In the interim, I've created an experimental `ImGui_Translator` class contains a buffer that gets `ImDrawData` copied into it every frame. Whilst this still relies on the DX11, there could be room to handle the draw data via GML but due to GameMaker's lack of support for index buffers this seems to be a bigger task than expected.
